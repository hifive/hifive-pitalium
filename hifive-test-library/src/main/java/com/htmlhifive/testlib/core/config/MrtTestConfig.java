 ...
 * テスト全体の設定情報を保持するクラス
 */
public class MrtTestConfig {

	private static final Logger LOG = LoggerFactory.getLogger(MrtTestConfig.class);
	private static final String SYSTEM_STARTUP_ARGUMENTS_PREFIX = "com.htmlhifive.testlib.";

	private static MrtTestConfig instance;
	 ...
	 */
	@VisibleForTesting
	MrtTestConfig(Map<String, String> startupArguments) {
	...
	/**
	 * {@link MrtTestConfig}のインスタンスを取得します。
	 * 
	 * @return インスタンス
	 */
	public static synchronized MrtTestConfig getInstance() {
		if (instance != null) {
			return instance;
		}

		instance = new MrtTestConfig();
		return instance;
	}
	...
	public <T> T getConfig(Class<T> clss, String name) {
		synchronized (configs) {
			...
			MrtConfiguration configuration = clss.getAnnotation(MrtConfiguration.class);
			if (configuration == null) {
				throw new TestRuntimeException("Configuration class must be annotated with @MrtConfiguration");
			}
	...
	/**
	 * {@link MrtConfigurationProperty}が設定されているフィールドに対し値を設定します。
	 * 
	...
	private static void fillConfigProperties(Object object, Map<String, String> arguments) {
		...
		for (Field field : fields) {
			MrtConfigurationProperty propertyConfig = field.getAnnotation(MrtConfigurationProperty.class);
			if (propertyConfig == null) {
				MrtConfiguration config = field.getAnnotation(MrtConfiguration.class);
				if (config == null) {
					continue;
				}
	...
	private <T> T loadConfig(Class<T> clss, String fileName, String defaultFileName) {
		...
		if (!Strings.isNullOrEmpty(fileName)) {
			...
			try {
				...
				try {
					return JSONUtils
							.readValue(MrtTestConfig.class.getClassLoader().getResourceAsStream(fileName), clss);
		...
		// 取得に失敗した場合何もしない
		try {
			return JSONUtils.readValue(MrtTestConfig.class.getClassLoader().getResourceAsStream(defaultFileName), clss);
		} catch (JSONException e) {
