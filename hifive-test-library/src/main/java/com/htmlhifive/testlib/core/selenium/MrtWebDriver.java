...
import com.htmlhifive.testlib.core.model.ScreenAreaWrapper;
 ...
 */
public abstract class MrtWebDriver extends RemoteWebDriver {
	 ...
	 */
	protected MrtWebDriver(URL remoteAddress, MrtCapabilities capabilities) {
		super(remoteAddress, capabilities);
		this.capabilities = capabilities;

		// JsonToWebElementConverterを上書きすることでfindElementから取得されるRemoteWebElementを差し替え
		setElementConverter(new JsonToMrtWebElementConverter(this));
	}

	/**
	 * テスト対象アプリケーションのベースURLを取得します。
	 * 
	 * @see com.htmlhifive.testlib.core.config.TestAppConfig#baseUrl
	...
	public ScreenshotResult takeScreenshot(String screenshotId, List<CompareTarget> compareTargets,
		...
		List<MrtWebElement> hiddenElements = findElementsByDomSelectors(hiddenElementSelectors);
	...
	protected ScreenshotImage getScreenshotImage(ScreenshotParams params, ScreenshotParams... additionalParams) {
		...
		if (isHideElementsRequired()) {
			for (MrtWebElement element : params.getHiddenElements()) {
				element.hide();
			}
		}
	 ...
	 * @return 取得した要素のリスト
	 */
	protected List<MrtWebElement> findElementsByDomSelectors(List<DomSelector> selectors) {
		List<MrtWebElement> elements = new ArrayList<MrtWebElement>();
		...
		for (DomSelector selector : selectors) {
			for (WebElement element : selector.getType().findElements(this, selector.getValue())) {
				elements.add((MrtWebElement) element);
			}
		}
		return elements;
	}
	 ...
	 * @return WebElement
	 */
	protected abstract MrtWebElement newMrtWebElement();

	//</editor-fold>

	//<editor-fold desc="JsonToMrtWebElementConverter">

	/**
	 * JSONデータをWebElementに変換するクラス
	 */
	static class JsonToMrtWebElementConverter extends JsonToWebElementConverter {
		 ...
		 */
		JsonToMrtWebElementConverter(MrtWebDriver driver) {
			super(driver);
			this.driver = driver;
		}

		@Override
		protected MrtWebElement newRemoteWebElement() {
			MrtWebElement element = driver.newMrtWebElement();
