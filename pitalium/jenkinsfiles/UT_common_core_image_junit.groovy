 ...
 * 単体テスト（common, core, image, junitパッケージ以下）を実行する。
 */
node {
	...
	withEnv(["ANT_OPTS=-Dant.proxy.host=${ANT_PROXY_HOST} -Dant.proxy.port=${ANT_PROXY_PORT}"]) {
		bat("${antHome}/bin/ant.bat -file pitalium/ci_build.xml unit_test && exit %%ERRORLEVEL%%")
	}

	stage('Archive Artifact')
	// カバレッジレポートが集約時に競合しないよう、ファイル名を変更
	bat('''set browserName=%JOB_NAME:Pitalium/=%
