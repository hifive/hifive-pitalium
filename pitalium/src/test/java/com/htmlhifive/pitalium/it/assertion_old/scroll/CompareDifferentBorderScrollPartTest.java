...
public class CompareDifferentBorderScrollPartTest extends PtlTestBase {
	...
	public void compareDifferentBorderWidth() {
		...
		if (config.getEnvironment().getExecMode() == ExecMode.RUN_TEST) {
			driver.executeJavaScript("document.getElementById('about-scroll').style.border = '3px solid black';");
	...
	public void compareDifferentBorderWidth2() {
		...
		if (config.getEnvironment().getExecMode() == ExecMode.RUN_TEST) {
			driver.executeJavaScript("document.getElementById('about-scroll').style.border = '1px solid red';");
			thrown.expect(AssertionError.class);
		}
